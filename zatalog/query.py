...
def _get_part(value: Any, part: str, so_far: str) -> Any:
    if isinstance(value, Entity):
        ...
        raise KeyError(
            f"Entity has no field '{part}' (at '{so_far}'); fields: {', '.join(_ENTITY_FIELDS)}"
        )
    if isinstance(value, EntityMetadata):
        if part in _METADATA_FIELDS:
            return getattr(value, part)
        raise KeyError(
            f"metadata has no field '{part}' (at '{so_far}'); fields: {', '.join(_METADATA_FIELDS)}"
        )
    ...
    if isinstance(value, list):
        ...
        try:
            ...
            raise KeyError(
                f"'{part}' is not a valid list index (at '{so_far}'); valid indices: {bounds}"
            ) from e
        try:
            return value[index]
        except IndexError as e:
            raise KeyError(
                f"Index {index} out of range (at '{so_far}'); valid indices: {bounds}"
            ) from e
    raise KeyError(f"Cannot look up '{part}' on {type(value).__name__} (at '{so_far}')")


def resolve_annotation(
    catalog: Catalog, entity: Entity, key: str, walk: bool = True
...
def available_annotations(
    catalog: Catalog, entity: Entity, walk: bool = True
...
def jira_info(
    ...
    return JiraInfo(
        projects=projects, component=component, label=label, source=source, raw=raw
    )
